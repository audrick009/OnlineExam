...
class Jobs extends CI_Controller {
	...
	public function apply() {
		...
		$this->form_validation->set_rules('essayAns', 'Essay', 'trim');

		if($this->jobs_model->checkIfTakenExam($app_id) == "true") {
			if($this->jobs_model->checkApplicantJobs($app_id, $this->input->post('job_id'))) {
				...
				if($this->form_validation->run()) {
					...
					$this->jobs_model->applyJob($job);
                                        $this->jobs_model->changeToPending($app_id);
			...
			echo "<script>alert('Uh oh! Please take or finish the examination first...');</script>";
			redirect('applicant/jobs', 'refresh');
		}
	...
	public function check() {
		print_r($this->jobs_model->checkIfTakenExam(11));
	}
}
?>